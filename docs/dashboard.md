...
- `/` : 허브 페이지
- `/project.html` : 로컬 기반 프로젝트 개선 토론
- `/news.html` : 뉴스 근거 수집과 뉴스 토론
- `/settings.html` : 역할 템플릿 YAML 편집

분리 이유는 단순하다.

- 프로젝트 개선은 로컬 실행 경로, 첨부 파일, 코드 문맥이 중심이다.
...
- 프로젝트 개선과 뉴스 토론을 서로 다른 페이지로 분리했다.
...
- 프로젝트 페이지는 `codex / claude / gemini`만 허용
...
- 프로젝트 페이지와 뉴스 페이지 모두 라운드 탭을 누적해서 보여주며, 이전 라운드 보드를 다시 열 수 있다.
