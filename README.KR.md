...
- `/` : 허브 페이지
- `/project.html` : 로컬 프로젝트 개선 + 신규 기획 토론 워크벤치
- `/news.html` : 뉴스/웹 근거 수집 + 근거 기반 토론 워크벤치
- `/settings.html` : 역할 템플릿 YAML 편집

핵심 흐름:

- 프로젝트·기획 페이지는 로컬 작업 디렉터리, 첨부 파일, 아이디어 스튜디오 브리프를 기준으로 토론을 실행합니다.
...
- 프로젝트·기획 페이지는 기본적으로 CLI provider(`codex`, `claude`, `gemini`)를 사용하고, 프로젝트 자동 컨텍스트를 끈 기획/아이디어 세션에서는 `ollama`도 허용합니다.
